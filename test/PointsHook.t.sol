...
contract TestPointHook is Test, Deployers {
    ...
    MockERC20 token; // token tot use ETH-TOKEN pool

    // native currency are represented by zero address
    ...
    function setUp() public {
        ...
        //Mint a bunch of token to yurself
        token.mint(address(this), 1000 ether);

        //Deploy hook to an address that has proper flag set
        uint160 flags = uint160(Hooks.AFTER_ADD_LIQUIDITY_FLAG | Hooks.AFTER_SWAP_FLAG);

        // deploying code to set address
